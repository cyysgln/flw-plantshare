...
import com.flower.component.fileUpload.enums.UploadResultEnum;
import com.flower.modules.system.domain.Upload;
import com.flower.modules.system.service.UploadService;
...
public class UploadController {
    ...
    @ResponseBody
    public ResultVo uploadFlowerImage(@RequestParam("flower") MultipartFile multipartFile) {

        // 创建Upload实体对象
        Upload upload = FileUpload.getFile(multipartFile, "/flower");
        try {
            return saveImage(multipartFile, upload);
        } catch (IOException | NoSuchAlgorithmException e) {
            return ResultVoUtil.error("上传图片失败");
